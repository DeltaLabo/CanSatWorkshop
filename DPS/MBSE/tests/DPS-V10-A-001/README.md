...
- **Model elements covered:** DPS v1.0 component exchanges listed in `../README.md` §3.3, including paired CE-03a ground-station Forwarder→Decoder UART telemetry and CE-03b Decoder→Forwarder UART command exchanges.
...
| View | Purpose |
|---|---|
| [`DPS-V10-A-001_view1_component_exchange_analysis_context.d2`](./DPS-V10-A-001_view1_component_exchange_analysis_context.d2) | PV2 context showing modeled CEs, including paired CE-03a/CE-03b UART telemetry/command, the verification-only CE Analysis Record LC, analyst/reviewer EAs and analysis constraints. |
...
4. **Directionality:** data/control direction matches the selected model intent, including Decoder fan-out to CSV Writer and Dashboard, Dashboard-to-Decoder command flow, and CE-03 UART evidence for both Forwarder→Decoder telemetry bytes and Decoder→Forwarder command bytes or an approved paired-UART representation.
...
| CE-03a / CE-03b | LoRa Forwarder → PC Decoder UART telemetry and PC Decoder → LoRa Forwarder UART command | Check ground forwarder-to-PC telemetry framing and PC-to-forwarder command framing, baud/configuration and byte ownership across the USB serial/UART bridge. | UART/USB-serial evidence supports both telemetry bytes from Forwarder to Decoder and command bytes from Decoder to Forwarder; baud/framing/message delimiter or packetization is documented; stale bytes, corrupt frames, duplicate/stale commands and disconnects are rejected/logged. | Forwarder serial code, PC decoder serial code, command queue/serial-write code, bidirectional serial logs, baud/framing configuration, example telemetry and command captures. |

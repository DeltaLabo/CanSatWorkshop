...
| SUB-ADS-002 | ADS | ADS v1.0 internal delivery/update-rate closure is inconsistent. | ADS README requires at least 5 Hz internal delivery to OBCC, while ADS `ADS-IVV-C-RATE-5HZ` treats v1.0 as N/A. | Add or clarify v1.0 ADS-to-OBCC update-rate test. |
...
| SUB-AMS-002 | AMS | Temperature response-time requirement is not clearly closed. | README requires temperature response time ≤60 s; current plans emphasize accuracy, exposure, I2C timing, and processing timing. | Add explicit temperature step-response test or trace it to an existing activity. |
| SUB-AMS-003 | AMS | AMS ≥5 Hz delivery to OBCC is not explicitly closed. | Project rate semantics mention internal collection, but AMS final tests do not clearly prove v1.0 AMS-to-OBCC update rate ≥5 Hz. | Add AMS-to-OBCC rate/freshness test. |
...
| SYS-004 | `SYS-END-TO-END-DATA` | Verify complete data path from truth/reference stimulus to ground evidence. | Sensor truth → subsystem processing → OBCC payload → LoRa → DPS CSV/dashboard; units, timestamps, freshness, stale/error flags. |

...
| Candidate / update ID | Selected path | Complete oracle and predeclared evidence expectations | Current status |
|---|---|---|---|
| `AMS-V10-TEMP-RESPONSE-60S` | Phase-2 candidate/reference only; no folder is created in this phase. Use a standalone modeled activity, or explicitly extend `AMS-VV-FC-001`, before claiming response-time closure. | Apply a declared temperature step; define final/reference condition and response-time calculation; verify reported temperature reaches the defined condition within `<=60 s`; predeclare and retain raw AMS timestamps/data, synchronized or monotonically correlated reference thermometer data, stable thermal source/chamber or fixture description, UUT/firmware/configuration IDs, sunlight/airflow exposure state, uncertainty/deviation notes, and calculation script/version. | Candidate/update path selected for phase 2; not created or closed here. |
| `AMS-V10-DATA-FRESHNESS` | Existing-test update alias through `AMS-VV-API-001`, `AMS-VV-FC-001`, `AMS-VV-FC-002`, `AMS-VV-CON-003`, and `AMS-VV-CON-004`; do not create a new folder unless a future controlled decision finds the extensions unclear. | Use the shared contract: `5 Hz` OBCC request/response (`200 ms` nominal), separate `2 s` LoRa telemetry packaging, fresh only when `status == VALID` and `age_ms <= 400 ms`, exact enum `VALID`/`STALE`/`NO_DATA`/`TIMEOUT`/`SENSOR_FAULT`/`INIT_FAIL`, timestamp/sequence/sample-ID/age evidence at the AMS-to-OBCC observation point, and no timeout/fault/init/no-data/stale condition leaving old/default/previous samples marked `VALID`. | Selected definition baseline applied to existing READMEs, D2 views, and refreshed v1.0 baseline copies in this phase; execution/reporting remains pending. |

Only the phase-2 temperature response-time activity remains unmodeled here. The freshness/status extensions above are no longer Markdown-only planning text in this AMS test package.

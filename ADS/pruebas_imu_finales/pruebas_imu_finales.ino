...
// Son 4 números (q0, q1, q2, q3) que describen una rotación sin sufrir gimbal lock (problema matemático con ángulos de Euler cerca de ±90°).
...
void applyIMUConfig() {
  // ----- ACELERÓMETRO -----
  myIMU.setAccRange(ICM20948_ACC_RANGE_2G);    // Rango de medición: ±2g (reduce ruido de alta frecuencia)
  myIMU.setAccDLPF(ICM20948_DLPF_6);             // Filtro digital paso-bajo nivel 6 (reduce ruido de alta frecuencia)
  myIMU.setAccSampleRateDivider(10);             // Divisor de tasa de muestreo → ~100 Hz

  // ----- GIROSCOPIO -----
  myIMU.setGyrRange(ICM20948_GYRO_RANGE_250);    // Rango: ±250 grados/segundo (mejor resolución angular)
  ...
  // ----- MAGNETÓMETRO -----
  myIMU.setMagOpMode(AK09916_CONT_MODE_100HZ);   // Modo continuo a 100 Hz (toma muestras automáticamente)
}
...
// Si arrancáramos con q = (1,0,0,0) (orientación "neutra"), el filtro tardaría unos segundos en converger a la orientación real. Para que arranque ya
// reflejando la posición física del sensor, calculamos roll/pitch/yaw iniciales a partir de accel+mag y construimos el cuaternión correspondiente.
void initQuaternionFromSensors() {
  ...
  for (int i = 0; i < N; i++) {
    myIMU.readSensor();
    myIMU.getGValues(&a);    // Accel en unidades de "g" 
  ...
  // Fórmula estándar de cuaternión como producto de 3 rotaciones (roll, pitch, yaw)
...
void madgwickUpdate(float gx, float gy, float gz,
  ...
  if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)) &&
    ...
    // Solo importa la DIRECCIÓN del vector gravedad/magnético, no su magnitud.
    ...
    // Para no repetir multiplicaciones muchas veces
    _2q0mx = 2.0f * q0 * mx;
    _2q0my = 2.0f * q0 * my;
    _2q0mz = 2.0f * q0 * mz;
    _2q1mx = 2.0f * q1 * mx;
  ...
  // Los cuaterniones que representan rotaciones deben tener magnitud 1. Por errores numéricos se va alejando de 1, así que lo renormalizamos cada ciclo.

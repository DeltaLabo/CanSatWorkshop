...
- **Constraints:** C01 max 500 m datalogger-ground separation; C02 concurrent Decoder/CSV/Dashboard execution; C03 browser-accessible dashboard; C04 unique CSV name from initial timestamp; C05 top alert when latency >1 s; C06 5 s toggle cooldown; C07 4x4 plot matrix with the CanSat attitude model in the top-left cell.
...
## 4. Historical v0.1 IVV activity inventory (deferred scope)
...
| DPS-V01-C-006 | DPS-v0.1 | C06 5 s cooldown | constraint | T | Feared command spam | P0,R1,R2,R3 | Burst trials and timing distribution; clock-jitter hardening. | 30 bursts; only first toggle enqueues; UI disabled during cooldown and re-enabled at 5.0 s ±0.5 s. | `tests/results/DPS-V01-C-006/` | Historical/deferred; not active CON-003 definition work |
| DPS-V01-C-007 | DPS-v0.1 | C07 4x4 plots, attitude model top-left | constraint | I/D | Telemetry visualisation use case | P0,R1,R2,R6 | Layout/mapping inspection; missing fields and resize hardening. | Required plot matrix is present; top-left cell renders CanSat attitude from pitch/roll/yaw; remaining dashboard cells are mapped to selected telemetry displays. | `tests/results/DPS-V01-C-007/` | Historical/deferred; not active CON-003 definition work |

## 5. Gaps before detailed definition

- Formal SSIV, mission, capability, use-case and feared-event IDs are missing.
- Historical v0.1 rows are deferred and are not active CON-003 definition work; if promoted later, define them using the active v1.0 DPS oracles.

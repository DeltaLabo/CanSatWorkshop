...
| `OBCC-V05-GATE-001-T02` | v0.5 -> v0.6 | On-mode state metadata, accepted ADS trigger, modeled emergency request, deployment condition/emergency evaluation, open command, PWM output, current/position evidence, `deployment_status` telemetry | Functional chain/scenario; constraint | Testing / Demonstration | `OBCC-UC-DeploymentGate`; `OBCC-FE-NonDeployment`; On-mode `<=5 s` opening constraint | Stat: plan `59/59` in-limit On-mode timing trials where feasible for 95/95 deadline screening; trigger and emergency-request cases separated or explicitly dispositioned. Fault hardening: telemetry success must be `OPEN_CONFIRMED` and must agree with independent current/position/PDM observer evidence; `COMMAND_SENT` is not success. | `PF-003`, `PF-007`, `PF-008`, `PF-009` | PV2, PV3, On-mode chain; expected report `results/OBCC-V05-GATE-001/onmode_opening_timing.md` | Definition ready; execution pending |
...
- **PF-003 — On-mode opening timing:** for valid ADS trigger or modeled emergency request while On, observed safe-actuator open position occurs `<=5.0 s` after the accepted trigger/request timestamp; telemetry success is reported only as `deployment_status = 4` / `OPEN_CONFIRMED` and agrees with current/position/PDM observer evidence. `COMMAND_SENT` or `OPEN_IN_PROGRESS` is timing-progress evidence only, not success.
- **PF-004 — invalid/missing input hardening:** invalid trigger/emergency requests and missing/stale ADS data are observable, do not command open in a forbidden context, and do not report deployment success without observer evidence. Expected telemetry status is `NOT_COMMANDED`, `INHIBITED_STANDBY`, `NO_OPEN_CONFIRMED`, `UNKNOWN`, or an applicable fault value, never `OPEN_CONFIRMED` unless open is independently confirmed.
- **PF-005 — reset/actuator-fault hardening:** reset during actuation and servo current/position faults are observable in trace/telemetry; no false success is allowed. `TIMEOUT`, `JAM_DETECTED`, `PDM_FAULT`, `UNKNOWN`, or safe/error/degraded status is recorded where applicable and none of these statuses is deployed/success.
- **PF-006 — exchange/allocation consistency:** implemented mode gate, ADS IMU/alt data path, emergency-request path, PDM/PWM actuation path, actuator feedback, telemetry/status publication, and verification-only harness interfaces match the modeled allocation and exchanges; no unmodeled bypass can command open around Stand-by inhibit. Reports shall map implementation-specific actuator/PDM feedback fields to the controlled `deployment_status` enum.
...
- Fault-injection coverage is by declared equivalence class: invalid/out-of-context trigger, invalid/out-of-context emergency request, missing ADS data, stale ADS data, reset during actuation, servo jam/current fault, and telemetry/observer mismatch. Each forbidden event is a failure. The expected status sequence, raw status codes/names, and any fault/reject reasons shall be predeclared for each class.
...
- Fault-hardening emphasis: Stand-by emergency-deploy inhibition, invalid/missing ADS data, reset during actuation, actuator-current/position anomalies, scheduler/trace liveness, and avoidance of false deployment-success telemetry. Only `OPEN_CONFIRMED` may close the deployed branch; inhibited, no-open, command-sent, in-progress, timeout, jam, PDM fault, and unknown states remain distinguishable in telemetry and reports.
...
- Emergency-deploy policy and actuator-status semantics are controlled by `OBCC-V10_Deployment_Fault_Policy.md`; execution still records implementation-specific field mappings, reset-during-actuation behavior, and any deviations.
- Safe fixture design, current limit, actuator surrogate/servo model, ambient thermometer model/calibration, parser/decoder version, and telemetry `deployment_status` byte mapping are execution records that must be recorded before strict credit.

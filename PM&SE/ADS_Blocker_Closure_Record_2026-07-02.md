...
This record captures definition-blocker resolution decisions for ADS verification planning only. It does not execute tests, does not create execution reports, and does not claim pass/fail credit. Later issues must update the affected ADS test definitions, local schema/contract artifacts, D2/PNG views where needed, and execution reports.
...
|---|---|---|---|---|---|
| ADS-BLK-001 | v1.0 ADS shall provide internally available data to OBCC at 5 Hz. OBCC LoRa telemetry remains a separate 2 s cadence and samples the latest ADS data with age/stale/error metadata. The current v1.0 N/A rationale for the ADS 5 Hz verdict is superseded. | Add/update a v1.0 ADS-to-OBCC rate/freshness candidate, preferably by extending `ADS-IVV-C-RATE-5HZ` plus `ADS-IVV-FC-OBCC-DELIVERY` rather than adding a duplicate folder. | `ADS-IVV-C-RATE-5HZ`, `ADS-IVV-FC-OBCC-DELIVERY`, `ADS-IVV-FC-MISSION-WINDOW`, ADS test index. | ADS/OBCC contract, OBCC log harness, monotonic timestamps or equivalent, firmware build ID, UUT ID, equipment IDs/calibration, and timing-analysis script revision. | Definition decision closed here; README/D2/test-definition update and execution follow-up remain. |
...
| ADS-BLK-007 | v1.0 `Pointers`/`Returns` requires a controlled ADS/OBCC contract with field/schema semantics, pointer ownership/lifetime, return/error codes, freshness, and watchdog/timeout threshold. | No duplicate candidate if `ADS-IVV-FC-OBCC-DELIVERY` is extended; otherwise create a contract/watchdog companion candidate. | `ADS-IVV-FC-OBCC-DELIVERY`, `ADS-IVV-C-RATE-5HZ`, `ADS-IVV-C-NOBLOCK`, ADS test index. | Local contract/header/code baseline, OBCC delivery harness, freshness timestamps, watchdog logs, fault injection for null/stale/corrupt/slow calls, firmware commit. | Decision captured here; local contract/code artifact and D2/test-definition follow-up remain before execution. |
...
|---|---|---|---|
| `ADS-IVV-C-RATE-5HZ-V10-OBCC` or extension of `ADS-IVV-C-RATE-5HZ` / `ADS-IVV-FC-OBCC-DELIVERY` | Close v1.0 ADS-to-OBCC internal delivery-rate/freshness. | ADS shall make an ADS update record/slot available to OBCC at 5 Hz for the 10 min mission window. OBCC LoRa remains 2 s and carries latest ADS values plus age/stale/error metadata. Missing, duplicate, corrupt, or stale-as-valid records fail the strict delivery claim. | Prefer updating the two existing activities and ADS test index; add D2 definition views only if the existing activities become too overloaded. |
...
No additional ADS planning candidates are selected by this record. Serial0 schema/parser and ADS/OBCC contract files are definition artifacts for existing activities, not additional mission-behavior candidates.

## Existing ADS test update list

| Existing activity | Required update for later issues |
|---|---|
| `ADS-IVV-C-RATE-5HZ` | Replace the v1.0 N/A-only rationale with an applicable v1.0 internal ADS-to-OBCC rate/freshness branch, or explicitly reference the v1.0 branch in `ADS-IVV-FC-OBCC-DELIVERY`. Add the 200 ms nominal period, `3000` expected updates over 10 min, jitter/sample rule, worst-gap rule, stale/drop/duplicate criteria, and instrumentation-overhead handling. |
...
| `ADS-IVV-FC-OBCC-DELIVERY` | Reference the controlled v1.0 ADS/OBCC `Pointers`/`Returns` contract. Add 5 Hz delivery/freshness semantics, watchdog thresholds, pointer ownership/lifetime, return/error codes, freshness age, stale/error flags, and null/dangling/stale/corrupt/slow-call fault cases. |
...
- v1.0 OBCC LoRa telemetry remains **2 s**. The LoRa frame may carry the latest ADS record plus `age`/freshness/error metadata; it is not the 5 Hz internal delivery evidence.
- Delivery freshness/watchdog thresholds selected for v1.0:
  - age `<= 400 ms`: usable as fresh for internal OBCC consumers, subject to field validity;
  - age `> 400 ms`: mark stale/degraded and fail strict freshness if uncorrected in the rate run;
  - age `> 1 s`: delivery fault/health error; OBCC shall not consume as valid fresh ADS data.
...
1. Update `ADS-IVV-C-RATE-5HZ` definition views/README and the ADS test index so v1.0 is no longer N/A for internal ADS-to-OBCC 5 Hz delivery, or add a clearly linked v1.0 rate/freshness definition view.
...
4. Create the ADS/OBCC `Pointers`/`Returns` contract/header/code baseline with watchdog and freshness semantics, and reference it from `ADS-IVV-FC-OBCC-DELIVERY`.
...
- Exact ADS/OBCC `Pointers`/`Returns` field schema, header/API names, watchdog implementation, and OBCC log harness.

...
## Approved S&A tailored environmental-screen baseline (`SAA-BLK-003`)

This activity defines the S&A tailored environmental screen as an article/workmanship screen for the screened article only. It does not claim qualification, population reliability, or `SYS-ENVIRONMENTAL-REGRESSION` phase-3/system-level environmental regression.

- **Article state and pre-screen evidence:** use the full `SAA-v1.0` as-flown article with battery, PCBs, backplane, headers/connectors, cap, parachute, servo, retention hardware and any required ballast installed. The only environmental-screen exception is the explicit `SAA-v0.1` structural/ballasted transition-readiness drop in `SAA-VV-CON-001`; that exception is limited evidence and is not final v1.0 CON-005 retention credit. Before execution, predeclare article/build IDs, installed state, ballast/surrogate rationale, fixture adapters, safety boundary, equipment calibration/status, ambient, camera/logger setup and evidence path. Perform a 100% pre-screen PC/PL/interface census plus witness-mark/gap photos. Do not tighten, reseat or rework between pre/post checks unless an anomaly is opened and affected events/checks are re-run.
- **Quasi-static/proof retention:** apply `20 g` equivalent inertial proof load in `+X`, `-X`, `+Y`, `-Y`, `+Z`, `-Z` where safe/meaningful. Use minimum `20 N` for the ESS battery pack/fixture and minimum `10 N` for PCBs, backplane/header/connector retention points, and top-module/PDM-servo-connector features. Hold `10 s` at target and record load/time or load-cell data, adapter, axis/orientation, and mass/surrogate rationale. Use surrogate fixtures if direct loading would be unsafe and record limitations.
- **Low-level vibration/handling:** execute only controlled padded fixture, shaker or handheld non-destructive events. Cover all three axes with positive/negative mounting or gravity orientations where coupling matters, `60 s` per approved axis/orientation, with accelerometer/event-log evidence. If no calibrated level is available, the event is limited/characterization only and cannot replace the `20 g` proof or drop screens. Abort on detachment, crack, connector unseat, battery movement, rail/continuity loss, unsafe fixture behavior, over-test, or logger/camera loss.
- **Landing/drop impact:** use a flat plywood or hardwood impact plate over rigid floor/concrete and record material, thickness and support. Drop from `1.0 m` free fall measured from the article lowest point to the surface. For `SAA-VV-CON-005`, execute at least `3` landing-style drops: bottom/longitudinal, side/lateral and top/cap worst-case; use all `6` `+X`, `-X`, `+Y`, `-Y`, `+Z`, `-Z` orientations for broad coverage when approved. Do not add intentional spin, hand assistance or preload; capture full video. Invalid release, video or fixture error may be re-run only if no damage occurred; damage or threshold exceedance opens an anomaly.
- **Post-screen gates:** perform a 100% post-screen PC/PL/interface inspection. Apply the `SAA-BLK-007` thresholds above; verify post-screen envelope `D_max + U_D <= 66.0 mm`, `H_max + U_H <= 122.0 mm`; verify v1.0 mass `measured mass + U_M < 350.0 g`; verify no unexplained part loss; and verify post-screen backplane seating/continuity/isolation/rails pass (`5.0 ±0.1 V`, `3.3 ±0.1 V`) under recorded safe no-load or approved load.
...
| Tailored Vibration / Drop / Proof-load Fixture | Provide the approved S&A article/workmanship screen: `20 g` equivalent proof-load axes with `20 N` / `10 N` minimums and `10 s` holds, controlled `60 s` low-level vibration/handling events where used, `1.0 m` landing-style drops, safe coupling/adapters, remote release and safe stop. |
...
3. Predeclare installed state, ballast/surrogate rationale, fixture adapters, safety boundary, ambient temperature/humidity, accelerometer/event logger setup, camera field of view, instrument calibration/status and evidence path.
4. Perform mandatory 100% pre-screen PC/PL/interface inspection for battery fixture, stack screws, PCB standoffs/screws, backplane fixture, headers, PDM Servo Connector and top-module context. Capture witness marks, gap/torque marks and reference photos.
5. Configure quasi-static/proof-retention loading where safe/meaningful: `20 g` equivalent inertial proof load in `+X`, `-X`, `+Y`, `-Y`, `+Z`, `-Z`; minimum `20 N` for the ESS battery pack/fixture; minimum `10 N` for PCBs, backplane/header/connector retention points and top-module/PDM-servo-connector features; `10 s` hold at target. Record load/time or load-cell data, adapter, axis/orientation and mass/surrogate rationale.
6. Configure low-level vibration/handling only as a controlled padded fixture, shaker or handheld non-destructive event. Cover all three axes with positive/negative mounting or gravity orientations where coupling matters, `60 s` per approved axis/orientation, and accelerometer/event-log evidence. If no calibrated level is available, mark the event limited/characterization only.
7. Configure landing/drop impact with a flat plywood or hardwood plate over rigid floor/concrete; record material, thickness and support. Set `1.0 m` free fall from the article lowest point and configure at least `3` landing-style drops (bottom/longitudinal, side/lateral, top/cap worst-case), or all `6` `+X`, `-X`, `+Y`, `-Y`, `+Z`, `-Z` orientations for broad coverage when approved.
8. Couple the UUT without hidden preload, unapproved shim, obstruction, temporary omission, hand assistance, intentional spin or damage.
9. Execute the approved proof, low-level vibration/handling and/or landing-drop events and monitor acceleration/event logs, camera view, witness observations, over/under-test condition, aborts and forbidden events. Abort on detachment, crack, connector unseat, battery movement, rail/continuity loss, unsafe fixture behavior, over-test, or logger/camera loss.
10. Repeat the configured sequence for every approved axis, orientation, level or event in the screen definition. Invalid release, video or fixture error may be re-run only if no damage occurred.
11. Safe the fixture and remove the UUT without tightening, reseating or altering the as-found condition unless the anomaly chain has been opened; after approved disposition, re-run affected events/checks or the full screen as required.
12. Perform mandatory 100% post-screen retention inspection for detachment, looseness, witness-mark shift, new gap, rattle, crack, bent pin, connector unseat and hidden damage.
13. Measure or bound battery, PCB/standoff, backplane/header, connector/harness and stack/top-module/cap/parachute movement against the released thresholds above. If a feature cannot be measured to the required resolution, use before/after witness marks, macro photos and continuity/functional checks; ambiguous evidence is hold, not pass.
14. Check post-screen mass/part census and envelope compliance. Verify no retained part was lost, `D_max + U_D <= 66.0 mm`, `H_max + U_H <= 122.0 mm`, and `measured mass + U_M < 350.0 g` for the v1.0 article.
15. Run post-screen backplane/header/PDM-servo-connector seating checks, continuity/isolation and required rail/testpoint checks. Apply `5.0 ±0.1 V` and `3.3 ±0.1 V` only under recorded safe-load or no-load conditions when power is applied.
16. Apply pass/fail constraints and statistical/fault-hardening viewpoints. If any hold trigger occurs, execute the anomaly disposition chain, preserve as-found evidence, quarantine the article, disposition corrective action and re-run affected checks or full screen as required.
17. QA witness releases pass/fail/hold decision and the evidence repository archives the screen definition, limitations, logs, photos/video, inspection records, measurements, rail/interface records, anomaly dispositions and final decision.

## Pass/fail constraints

Pass only if all of the following are true for the `v1.0` article:

- the approved S&A article/workmanship screen is predeclared and as-run recorded before execution, including applicable `20 g` equivalent proof-retention axes with `20 N` / `10 N` minimums and `10 s` holds, `60 s` low-level vibration/handling per approved axis/orientation where used, and `1.0 m` landing-style drops on recorded plywood/hardwood impact surface where used;
...
- post-screen envelope remains compliant with `D_max + U_D <= 66.0 mm` and `H_max + U_H <= 122.0 mm`, and post-screen v1.0 mass remains compliant with `measured mass + U_M < 350.0 g`;
...
Fail or hold for any detachment, looseness, movement or seating-gap increase beyond the released thresholds, shifted witness mark beyond threshold, new gap beyond threshold, rattle, crack, bent pin, connector unseat, battery movement, intermittent continuity, new short/open, rail/testpoint failure, deployment-clearance interference, uncontrolled/unapproved screen event, under/over-test, fixture-induced hidden preload/damage, missing predeclared evidence, missing or ambiguous evidence, logger/camera loss, unapproved rework/tightening/reseating, unresolved anomaly, unapproved waiver, unresolved limitation or required re-run not completed.

## Viewpoints

- **Statistical significance:** This activity is a v1.0 article/workmanship environmental screen by default. It verifies the screened article/configuration and makes no population reliability or qualification claim. Pre/post inspection is a mandatory 100% census of every target PC, PL and interface on the screened article. Repeated axes/orientations/events on one article are workmanship/integration evidence, not independent reliability trials. If a separate reliability claim is later required, it requires an independently approved representative article/trial plan and project IVV binomial policy (for example, `29/29` zero-failure independent representative trials for an R90/C95 planning point).
...
- tailored screen definition and as-run record: article/workmanship-screen statement, proof-load type/level/axis/orientation/adapter/load-cell or load-time data/`10 s` holds, low-level vibration/handling `60 s` events and calibrated-or-characterization status, landing-drop `1.0 m` release details and impact-surface material/thickness/support, fixture coupling, abort criteria, limitations and approval/hold rationale;
...
- The executable activity is for `SAA-v1.0`; `v0.1` is retained as copied context only, with the explicit structural/ballasted transition-readiness drop exception handled by `SAA-VV-CON-001`.
- The environmental-screen levels, surfaces, orientations/axes, durations/events, limitations and success criteria are defined above by the approved tailored S&A article/workmanship screen. Execution still requires article IDs, equipment status, calibrated/approved evidence capture and as-run records before any pass can be claimed; this README update creates no execution credit.

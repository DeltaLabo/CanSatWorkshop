...
2. **Valid-frame statistics:** `N = 300` scheduled valid frames at `2.0 s ±0.2 s`; one-sided 95% exact Clopper-Pearson lower bound for decoded-and-stored PDR is `≥0.90` (`k ≥ 279` for `N = 300`).
3. **Latency and data integrity:** p95 RX-to-CSV latency `<1 s`; CSV rows equal accepted decoded frames; payload and sequence match transmitter log; CSV/session evidence uses the selected no-overwrite session naming, monotonic row order, and row-count/checksum/content match oracle.
...
The report in `tests/results/DPS-V10-T-001/` should reference these modeled diagrams and selected parameter/oracle baselines and record: model baseline, as-tested hardware/software configuration, transmitter log, CSV output, decoder/error logs, ambient reading, RF/radio setup, computed PDR/latency, CSV/session integrity comparison, corrupt-frame k/n by class, pass/fail rationale, deviations, anomalies, waivers and limitations.

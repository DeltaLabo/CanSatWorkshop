...
| `v1.0` | Applies. Source model includes ICM20948 on ADS PCB, I2C CE through the integrated ADS/backplane/OBCC path to ADS Processing, angular velocity chain functions, `Accuracy < 30 deg/s`, I2C timeout `<=5 ms`, and ADS Processing to OBCC `Pointers`/`Returns` delivery interfaces. | Pass if valid angular-rate data propagate through the modeled chain to ADS Processing and the integrated ADS/OBCC observation point where applicable, at least `n >= 30` matched valid ADS/truth samples exist per accepted axis/rate condition, the guarded absolute angular-rate error or formal 95/95 upper bound where claimed is `<30 deg/s`, every relevant I2C read returns data or timeout/error in `<=5 ms`, outputs remain finite/fresh/in range, and saturated/stale/non-finite/I2C-fault data are not accepted as valid by ADS Processing or OBCC. |

## Statistical and fault-hardening viewpoints

- **Statistical significance:** minimum `n >= 30` matched valid ADS/truth samples per axis/rate condition. If a formal 95/95 claim is made, record the tolerance/confidence method, sample independence assumptions, reference uncertainty, axis-alignment uncertainty, timebase uncertainty, and guard band.
- **Accuracy calculation:** compare ADS `gx/gy/gz` against the calibrated angular-rate reference or controlled still/rotation fixture after timestamp matching and validity filtering. The pass criterion is `<30 deg/s` after uncertainty/guard-band treatment.
...
Before execution, record ADS version, copied source-model package, hardware revision/BOM, firmware commit/build, UUT serial/identifier, logger configuration, reference fixture method, rate setpoints, axis/sign convention, mounting/alignment, warm-up, power mode, timebase, calibration status, ambient temperature/humidity, and all deviations.

Required verification means are modeled as: calibrated angular-rate reference or controlled still/rotation fixture, axis alignment fixture, ADS UUT/version selector, PC/OBCC logger, human test conductor, ambient monitor, statistical analysis tool, current-limited supply or battery simulator, and I2C/IMU fault injector.
...
- Exact angular-rate fixture model, ambient monitor model, calibration IDs, axis/rate matrix, timing tolerance method, and formal 95/95 calculation method must be fixed before execution.

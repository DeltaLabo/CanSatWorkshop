...
|---|---|---|---|
| `ADS-IVV-C-RATE-5HZ-V10-OBCC` or extension of `ADS-IVV-C-RATE-5HZ` / `ADS-IVV-FC-OBCC-DELIVERY` | Close v1.0 ADS-to-OBCC internal delivery-rate/freshness. | ADS shall make an ADS update record/slot available to OBCC at 5 Hz for the 10 min mission window using the shared PM&SE freshness/status contract. OBCC LoRa remains 2 s and carries latest ADS values plus age/status metadata. Missing, duplicate, corrupt, or stale-as-valid records fail the strict delivery claim. | Prefer updating the two existing activities and ADS test index; add D2 definition views only if the existing activities become too overloaded. |
...
- `PM&SE/contracts/sensor_obcc_freshness_contract.md` governs freshness/status for both ADS and AMS sensor-to-OBCC responses and 2 s telemetry packaging.
...
| `ADS-IVV-FC-OBCC-DELIVERY` | Reference the shared PM&SE sensor-to-OBCC freshness contract for 5 Hz delivery/freshness, 400 ms age, status enum, telemetry age/status, and no-old-data-as-`VALID` behavior. Add ADS-specific supplemental watchdog thresholds, pointer ownership/lifetime, return/error-code mapping, and null/dangling/stale/corrupt/slow-call fault cases. |

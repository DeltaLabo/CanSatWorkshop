...
# Add this import and function near the top of the file with the other imports
import socket
...
def get_local_ip():
    ...
    try:
        ...
        # Doesn't need to be reachable, just forces the socket to identify the interface
        s.connect(('8.8.8.8', 80))
        ip = s.getsockname()[0]
        s.close()
        return ip
    except:
        return '127.0.0.1'  # Fallback to localhost if cannot determine IP

# Add this after __HTTP_PORT and __SOCKET_PORT are defined
__MY_IP = get_local_ip()
...
def glowcomm_with_socket_port(port):
    global glowcomm_raw
    # provide glowcomm.html with socket number and IP address
    modified = glowcomm_raw.replace('XXX', str(port))
    # Replace all localhost references with the actual IP address
    modified = modified.replace('localhost', __MY_IP)
    return modified
    ...
    if platform.python_implementation() == 'PyPy':
        ...
        if not no_launch:
            if _browsertype == 'default':  # uses default browser
                _webbrowser.open('http://{}:{}'.format(__MY_IP, __HTTP_PORT))
    else:
        __server = HTTPServer(('', __HTTP_PORT), serveHTTP)
        # In the section where it launches the browser, replace the localhost references:
        if not no_launch:
            if _browsertype == 'default':  # uses default browser
                _webbrowser.open('http://{}:{}'.format(__MY_IP, __HTTP_PORT))
...
def start_Qapp(port, ip):
    # creates a python browser with PyQt5
    # runs qtbrowser.py in a separate process
    filepath = os.path.dirname(__file__)
    filename = filepath + '/qtbrowser.py'
    os.system('python ' + filename + ' http://{}:{}'.format(ip, port))

# And update the call to start_Qapp:
if _browsertype == 'pyqt':
    __m = multiprocessing.Process(target=start_Qapp, args=(__HTTP_PORT, __MY_IP,))
    __m.start()
...
def start_websocket_server():
    ...
    __factory = WebSocketServerFactory(u"ws://{}:{}/".format(__MY_IP, __SOCKET_PORT))
...
print(f"VPython server runnin at http://{__MY_IP}:{__HTTP_PORT}/")

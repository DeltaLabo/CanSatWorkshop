...
| PDM v1.0 | `PDM-VV-CE-001`..`PDM-VV-CE-005` | Candidates; I2C, Pointers/Returns, PWM, status-feedback, and torque-margin evidence remain pending. Servo status/no-false-success semantics for `CE-004` are selected at outline level. | Model CE analyses/tests using the selected status vocabulary and OBCC-owned policy boundary; execute later. |
...
| PDM-BLK-004 | **Closed at outline level** — servo status/no-false-success semantics are selected; future `PDM-VV-CE-004`, `PDM-VV-FC-001`, and OBCC/system tests must map implementation evidence to them. | No unresolved high-level status oracle; detailed status-field mapping, model updates, and execution evidence remain pending. | Use the selected status vocabulary and require `OPEN_CONFIRMED` or equivalent independent evidence for strict success. |
...
| `PDM-VV-CE-001`..`PDM-VV-CE-005` | Remain v1.0 candidates. | Later I2C, `Pointers/Returns`, PWM command, status-feedback, and torque-margin analyses/tests. `PDM-VV-CE-004` shall map implementation evidence to `NOT_COMMANDED`, `COMMAND_SENT`, `OPEN_IN_PROGRESS`, `OPEN_CONFIRMED`, `NO_OPEN_CONFIRMED`, `TIMEOUT`, `JAM_DETECTED`/`PDM_FAULT`, and `UNKNOWN`; only `OPEN_CONFIRMED` is strict success. |
...
| `PDM-VV-CE-004` future definition | Status feedback/no-false-success oracle selected; `COMMAND_SENT` is not success, and only `OPEN_CONFIRMED` is strict success. | Map implementation-specific fields, feedback, current/position, witness mark, or video evidence to the selected vocabulary. |

...
- **Model elements covered:** all six DPS v1.0 component exchanges listed in `../README.md` §3.3, with CE-03 ground-station UART selected as bidirectional model intent or paired telemetry/command UART exchanges while the source PV2/D2 update remains pending.
...
| CE-03 | LoRa Forwarder ↔ PC Decoder: UART selected intent; current source PV2 arrow update pending | Check ground forwarder-to-PC telemetry framing and PC-to-forwarder command framing, baud/configuration and byte ownership across the USB serial/UART bridge. | UART/USB-serial evidence supports both telemetry bytes from Forwarder to Decoder and command bytes from Decoder to Forwarder, or an approved paired-UART CE representation; baud/framing/message delimiter or packetization documented; stale bytes, corrupt frames, duplicate/stale commands and disconnects are rejected/logged. Unconditional CE-03 credit requires the source-model/D2 update or approved waiver to be recorded. | Forwarder serial code, PC decoder serial code, command queue/serial-write code, bidirectional serial logs, baud/framing configuration, example telemetry and command captures. |

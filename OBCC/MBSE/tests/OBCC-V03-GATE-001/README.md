...
- **Existing v1.0 candidate rows mapped:** `OBCC-VV-CE-002`, `OBCC-VV-FC-002`, `OBCC-VV-CON-001`.
...
| `OBCC-V03-GATE-001-T01` | v0.3 -> v0.4 | Telemetry collector, telemetry encoder, 34-byte target payload excluding relative humidity, health/status metadata, sequence/timestamp metadata | Functional chain/scenario; constraint | Testing + Analysis | `OBCC-UC-TelemetryDownlink`; `OBCC-VV-FC-002`; `OBCC-VV-CON-001` | Stat: field-by-field deterministic schema comparison over the planned frame set. Fault hardening: missing/stale values and RH absence are explicit, not silently misinterpreted. | `PF-001`, `PF-006` | PV2, PV3, Functional chain; expected report `results/OBCC-V03-GATE-001/frame_schema.md` | Definition ready; execution pending |
...
| `OBCC-V03-GATE-001-T03` | v0.3 -> v0.4 | PC decoder, dashboard, CSV writer, raw frame logger, malformed-frame injector, sequence/timestamp preservation | Functional chain/scenario; component exchange | Testing | `OBCC-FE-LostTelemetry`; `OBCC-VV-CE-002` | Stat: deterministic malformed-frame class coverage. Fault hardening: bad CRC, truncated, wrong-length, corrupt, and extra-RH frames are rejected and logged invalid. | `PF-003`, `PF-006` | PV2, PV3, Functional chain; expected report `results/OBCC-V03-GATE-001/decode_hardening.md` | Definition ready; execution pending |
| `OBCC-V03-GATE-001-T04` | v0.3 -> v0.4 | RFM95W LoRa downlink, ground station receiver/forwarder, RF path geometry, RSSI/SNR logging, packet counts and PDR evidence | Functional chain/scenario; constraint; component/link | Testing + Analysis | `OBCC-UC-TelemetryDownlink`; `OBCC-FE-LostTelemetry`; `OBCC-VV-CON-001` | Stat: PDR pass only when one-sided 95% exact binomial lower bound is `>=0.90` at `>=500 m` or justified RF equivalent; `96/100` valid frames is a planning point. Fault hardening: RF loss, bad antenna orientation, legal radio settings, weather/path effects, raw evidence retention. | `PF-004`, `PF-005`, `PF-006` | PV1, PV2, Functional chain; expected report `results/OBCC-V03-GATE-001/range_pdr.md` | Definition ready; execution pending; lab-only execution leaves range gate open |

## Pass/fail constraints

- **PF-001 — frame schema and payload content:** valid telemetry frames carry the target v1.0 34-byte measurement payload before frame/status metadata: pitch, roll, yaw; three angular speeds; three accelerations; altitude; temperature; latitude; longitude; battery voltage; battery current. Relative humidity is absent or explicitly error-coded and is never silently decoded as another field.
...
- **PF-003 — decode/display/store hardening:** the ground path preserves sequence and timestamps through raw logs, decoder, dashboard, and CSV. Corrupt, truncated, wrong-length, bad-CRC, or extra-relative-humidity frames are rejected and logged as invalid, not stored as valid telemetry.
- **PF-004 — range/PDR:** at `>=500 m` separation or a documented RF-equivalent setup, packet delivery passes only if the one-sided 95% exact binomial lower confidence bound is `>=0.90`. If only lab/bench evidence is available, schema/cadence/decode items may be characterized but the range/PDR gate remains open.
...
- PDR evidence uses the project IVV exact one-sided binomial policy. Reports shall record successes `k`, trials `n`, confidence, and lower bound; `96/100` valid received frames is a planning point for `PDR >= 0.90`.
...
- `LoRa_Frame.md` includes relative humidity and names `RFM96W`, while the v1.0 model excludes relative humidity and uses `RFM95W`; this gate treats RH exclusion and `RFM95W` as the v1.0 target context and requires any actual module substitution to be dispositioned.
...
- Range survey kit, weather/time reference, antenna-orientation tool, stimulus fixture, and analysis tool exact model/version identifiers are TBD and must be captured in evidence.

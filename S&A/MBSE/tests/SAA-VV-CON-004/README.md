...
| Manufacturing / Process Reviewer | Review slicer profile, printer setup, material lot and build logs; verify 15% infill or approved deviation; review batch drift, reprints and process evidence. |
...
| Printer Profile / Build Record Set | Provide slicer profile, G-code summary, print fill/infill value, material lot, printer ID, build date/time and affected part IDs. |
...
| Torque Driver / Insert-Cycle Tool and Cycle Counter | Apply controlled screw insertion/removal or representative assembly/fit cycles; record torque setting, tool ID, hole ID, screw type and cycle/trial count. |
| Ambient Thermo-Hygrometer | Record temperature and humidity before measurement or cycle repetitions; hold if ambient/material/tool state invalidates measurement or process interpretation. |
...
| Evidence Repository | Store controlled report, raw readings, calculations, profiles/build logs, screw census, photos/video, torque/cycle records, statistical-basis statement and anomaly dispositions under `S&A/MBSE/tests/results/SAA-VV-CON-004/`. |
...
1. Identify printer, material, nozzle/profile version, part IDs, reprint history and batch boundaries.
...
3. Collect slicer profile, G-code summary, printer log, material lot and build plate records.
4. Verify every accepted printed module, top-module part, screw-boss coupon or accepted surrogate has controlled evidence showing `15%` print fill/infill.
5. Treat non-`15%` infill as acceptable only when an approved controlled deviation exists before acceptance, tied to affected part IDs and re-verification scope.
6. Record missing logs, unapproved infill, part mix-up, material substitution, weak-infill concern, batch drift and any required hold/re-run.
...
2. Verify allowed screw type, torque driver identity/calibration and controlled torque/tool setting.
...
9. Pass requires no strip/crack/damage/alignment loss and complete evidence. Stop and hold on any damage, missing torque record or open anomaly.
...
- **Infill / print-fill:** printer/profile/build records show `15%` infill for every accepted printed module, top-module part, screw-boss coupon or accepted surrogate. A non-`15%` value requires controlled pre-acceptance deviation, part-ID scope and re-verification rationale.
...
- **Representative screw/fit cycles:** controlled torque/tool cycles produce no stripped boss, crack, delamination, weak-infill crushing, boss pull-out, torque damage, warped/blocked slot, loose standoff, blocked backplane/PCB fit or loss of stack/top-module alignment.
- **Evidence control:** baseline copy, article IDs, part/build IDs, printer/material batch IDs, screw inventory, equipment calibration, ambient record, raw measurements, uncertainty calculations, cycle/torque records, photos/video, witness review and anomaly/waiver/rework dispositions are complete.
- **Hold/fail triggers:** missing build logs, unapproved infill deviation, uncalibrated instruments, missing nominal values, unguarded measurements, forced fit, open anomaly, required re-run not completed, material/printer batch drift with no disposition, or evidence-control escape.
...
- printer/slicer/profile/G-code/build-log evidence, material lot, printer ID, batch IDs, `15%` infill records and any controlled deviations;
- critical-feature list, nominal source, raw measurements, expanded uncertainty values and `|error| + U` calculations;
- screw/fit-cycle plan, torque settings, cycle/trial counts, hole IDs, pre/post damage and alignment observations;
...
- Exact screw torque limits, screw-hole geometry tolerances, material/printer profile details and drawing nominals are assumed to be released before execution. If absent, the report records a hold rather than a pass for affected checks.

...
| The Energy Storage System  battery must use  2s 1p rechargeable LI-ion cells (3.7 nominal voltage). Rechargeable with an external port. | Battery Inspection |
| The  Energy Storage System must include voltage, current and power sensor for the batteries. | Power test |

### Success Criteria

The ESS demonstrates its ability to provide power for at least 6 hours in stand-by mode, includes essential battery monitoring sensors, and conforms to the specified charging and electrical configuration requirements.
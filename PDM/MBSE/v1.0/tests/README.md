...
| PDM-VV-FC-001 | PDM v1.0 | IMU-triggered parachute deployment functional chain; linked I2C/Pointers/PWM/Torque CEs; ADS/PDM/Trigger/Holder/Descent allocations; deployment-relevant PCs/PLs; source constraints | Functional chain/scenario with linked component exchange, allocation, component/link, and constraint coverage | Testing / Demonstration with supporting Analysis and Inspection | `PDM-UC-IMUTriggeredDeployment`; `PDM-UC-LimitTerminalSpeed`; `PDM-FE-NonDeployment`; `PDM-FE-PrematureDeployment`; `PDM-FE-SensorBusFault`; `PDM-FE-MechanicalJam`; linked constraints | IVV, PDM-R1–R16 | Stat: modeled ≥10 development nominal trials; R90/C95 release evidence requires 29/29 independent positive and 29/29 independent negative no-failure trials; 95/95 timing claims require 59/59 in-limit timings. Fault hardening: bad/stale IMU, I2C NACK/disconnect/stuck bus, false profiles, no-manual/BLE regression, servo jam/no-open, brownout/reset. | Pass iff nominal IMU profile produces exactly one ordered deployment chain; negative/manual/BLE attempts do not command open; I2C ≤5 ms; process/calculate <5 ms; no blocking outside I2C; initial servo 0 degrees/closed; angular-rate error <30 deg/s or robust threshold analysis; parachute diameter ≤30 cm; terminal-speed evidence recorded/linked; evidence/report content complete. | Modeled definition: `tests/PDM-VV-FC-001/README.md`; views: `tests/PDM-VV-FC-001/definition_views/PDM_VV_FC_001_view[1-5]_*.d2/png`; report/evidence: `tests/results/PDM-VV-FC-001/` | Modeled definition ready for review |
...
- **Pass/fail constraints:** nominal exactly-once deployment, negative/no-manual no-open behavior, I2C `≤5 ms`, process/calculate `<5 ms`, no blocking outside I2C, initial servo `0 degrees`/closed, angular-rate error `<30 deg/s` or robust-threshold analysis, parachute diameter `≤30 cm`, terminal-speed evidence recorded/linked, and complete report evidence.
...
- **Definition gaps:** PDM-VV-FC-001 now has a modeled definition, but final UUT ID, hardware revision, firmware commit/build, exact IMU stimulus/replay set, equipment calibration IDs, environmental limits, and statistical claim type remain execution prerequisites. Other candidate activities still need detailed definitions.

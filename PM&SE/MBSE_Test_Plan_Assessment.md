...
| GAP-004 | Payload schema | The DPS/OBCC RH field mismatch is closed, and ADS/AMS sensor freshness/status semantics are controlled by the shared PM&SE contract; broader schema versioning, payload fields, units, OBCC/DPS mapping, and dashboard behavior still need project-level control. | Freeze the remaining payload schema/dashboard behavior while preserving the shared ADS/AMS freshness contract. |

...
- **PF-003 — startup health:** nominal boot reaches Stand-by telemetry/heartbeat; startup-critical RFM95W, BME280, and Servo checks pass. Any critical startup fault blocks operational mode; non-critical faults are telemetry-visible and degraded only.
...
- The source context uses `RFM95W LoRa` and `ICM20948 IMU`; `LoRa_Frame.md` contains legacy names (`RFM96W`, `MPU6050`). Execution must record actual fitted hardware and disposition naming differences.

...
| SUB-SAA-003 | S&A | Environmental screen decisions are approved at definition-decision level. | Tailored drop/vibration/landing screen levels, surfaces, orientations/axes, durations/events, limitations, and success criteria are approved by `PM&SE/SAA_Blocker_Closure_Record_2026-07-02.md`; detailed S&A test-definition edit and execution remain pending. | Apply the pending `SAA-BLK-003` test-definition edit to affected S&A/system environmental planning, then execute/report before credit. |
...
| GAP-004 | Payload schema | The DPS/OBCC RH field mismatch is closed, DPS UI/CSV/latency/layout oracles are selected for detailed definitions, and ADS/AMS sensor freshness/status semantics are controlled by the shared PM&SE contract; broader schema versioning, payload fields, units, and OBCC/DPS mapping still need project-level control. | Freeze the remaining system payload schema and OBCC/DPS mapping while preserving the shared ADS/AMS freshness contract and selected DPS oracles. |
...
| STRAT-002 | Definition maturity | DPS parameter/oracle decisions are selected, but DPS and PDM still lag in detailed modeled definition completeness. | Prioritize detailed D2 definition completion for DPS candidate rows and PDM v1.0 candidate rows. |
...
| STRAT-004 | Fault hardening | OBCC, ADS, and PDS/ESS have strong explicit fault-hardening. DPS now has selected RF/UI/load hardening oracles but still needs detailed activity definitions; PDM needs more detailed fault definitions; S&A environmental-screen decisions are approved at definition-decision level but pending detailed test-definition edit and execution. | Add/complete detailed fault matrices, DPS activity definitions, PDM fault definitions, and the pending S&A environmental-screen definition edit/execution. |

...
### 5.1 AMS blocker alignment candidate/update paths

These Markdown planning rows align the AMS test package with the AMS blocker closure record. They do not create execution credit until the corresponding modeled D2 definitions and reports exist.

| Candidate / update ID | Selected path | Minimum oracle and evidence expectations | Current status |
|---|---|---|---|
| `AMS-V10-TEMP-RESPONSE-60S` | New AMS candidate for the README temperature response-time requirement; prefer a standalone modeled activity if the response oracle needs its own chain, otherwise extend `AMS-VV-FC-001` clearly. | Apply a declared temperature step and verify reported temperature reaches the defined final/reference condition within `<=60 s`; retain raw AMS timestamps/data, reference thermometer data, stable thermal source/chamber or fixture description, UUT/firmware/configuration IDs, sunlight/airflow exposure state, uncertainty/deviation notes, and response-time calculation. | Candidate/update path selected; D2/PNG modeling and execution remain follow-up. |
| `AMS-V10-DATA-FRESHNESS` | README-level update alias through `AMS-VV-API-001`, `AMS-VV-FC-001`, `AMS-VV-FC-002`, `AMS-VV-CON-003`, and `AMS-VV-CON-004`; create a new folder only if these extensions become unclear. | Use the shared contract: `5 Hz` OBCC request/response, `2 s` telemetry packaging, fresh only when `status == VALID` and `age_ms <= 400 ms`, exact enum `VALID`/`STALE`/`NO_DATA`/`TIMEOUT`/`SENSOR_FAULT`/`INIT_FAIL`, timestamp/sequence/age evidence, and no timeout/fault/init/no-data condition leaving old samples marked `VALID`. | Candidate/update path selected; Markdown READMEs reference the contract, while D2/PNG updates and execution remain follow-up. |

D2/model follow-up is intentionally limited to a later pass: create or update AMS test-definition D2 views for `AMS-V10-TEMP-RESPONSE-60S` and the freshness/status extensions above, then regenerate matching PNGs without changing this Markdown-only alignment record.

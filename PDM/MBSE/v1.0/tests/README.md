...
- **Servo/trigger status semantics (`PDM-BLK-004`):** future `PDM-VV-CE-004`, `PDM-VV-FC-001`, and no-false-success tests shall map implementation names to the meanings below. `OPEN_CONFIRMED` is the only strict success state; `COMMAND_SENT`, `OPEN_IN_PROGRESS`, `TIMEOUT`, `JAM_DETECTED`, `PDM_FAULT`, and `UNKNOWN` are not success by themselves.
  ...
  - `OPEN_CONFIRMED`: feedback or independent observer confirms open.
  ...
  Outline-level evidence options are servo feedback if implemented, PWM trace, current signature, position/witness mark, safe-fixture video, or explicit PDM feedback.
...
- **Pass/fail constraints:** nominal exactly-once deployment from fresh-valid ADS input under the OBCC policy, negative/no-manual no-open behavior, I2C `≤5 ms`, process/calculate `<5 ms`, no blocking outside I2C, initial servo `0 degrees`/closed, angular-rate error `<30 deg/s` or robust-threshold analysis, `OPEN_CONFIRMED` as the only deployment-success state, parachute diameter `≤30 cm`, terminal-speed evidence demonstrating or linking `<= 11 m/s`, and complete report evidence.

...
| `SYS-END-TO-END-DATA` | Truth/reference stimulus to DPS CSV/dashboard evidence. | Define canonical payload, units, timestamps, ADS/AMS freshness age/status using the shared PM&SE sensor-to-OBCC contract (`5 Hz` OBCC requests, `2 s` LoRa packaging, fresh age `<=400 ms`, enum `VALID`/`STALE`/`NO_DATA`/`TIMEOUT`/`SENSOR_FAULT`/`INIT_FAIL`, and no old data marked `VALID` after stale/no-data/timeout/fault/init-fail), plus cross-subsystem observability points. |

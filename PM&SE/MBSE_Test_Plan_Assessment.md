...
| SUB-DPS-004 | DPS | DPS/OBCC payload field blocker closed. | The obsolete RH dashboard/frame field was removed from the controlled OBCC table and DPS dashboard artifacts. | Broader payload-schema/versioning control is now handled by `PM&SE/contracts/obcc_dps_lora_telemetry_contract.md` and `SYS-END-TO-END-DATA`. |

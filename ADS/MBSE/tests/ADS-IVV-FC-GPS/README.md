...
- For `v0.1`/`v0.2`, record timing/rate/gap statistics against the modeled `At 5 Hz` constraint. The accepted timing rule is nominal 200 ms opportunities with no internal progress/freshness gap `>400 ms`; jitter is characterization unless it violates count/gap/freshness rules or a stricter declared test-specific limit.
